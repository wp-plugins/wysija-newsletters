...
class WYSIJA_model extends WYSIJA_object{
    ...
    function getRows($columns=false,$page=0,$limit=false){
        ...
        $query="SELECT ".$columns." FROM `".$this->getPrefix().$this->table_name."`";
    ...
    function count($query=false){
        if(!$query){
            $query="SELECT COUNT(".$this->getPk().") as count FROM `".$this->getPrefix().$this->table_name."`";
    ...
    function _secureFieldVal(&$field,&$val){
        ...
        if(is_string($val)) $val=mysql_real_escape_string($val);
            ...
            if(is_array($val)){
                foreach($val as $k=>$v){
                    if(is_string($v)) $val[$k]=mysql_real_escape_string($v);
                }
    ...
    function save($update=false){
        ...
        if($update){
            ...
            if($this->replaceQRY){
                ...
                $resultSave=$wpdb->insert($this->getPrefix().$this->table_name,$this->values,$fieldsFormats); 
            }
            global $wpdb;
        }
        if($this->dbg){
    ...
    function exists($conditions){
        ...
        $query="SELECT ".$this->getPk()." FROM `".$this->getPrefix().$this->table_name."`";
        if($this->setConditions($conditions)){
            $whereQuery=$this->makeWhere();
            if(!$whereQuery){
