...
class WYSIJA_model_campaign_list extends WYSIJA_model{
    ...
    function getReceivers($mailid,$total = true,$onlypublished = true){
            $query = 'SELECT a.name,a.description,a.published,a.color,b.listid,b.mailid FROM '.$this->getPrefix().'campaign_list as b LEFT JOIN '.$this->getPrefix().'list as a on a.list_id = b.list_id WHERE b.campaign_id = '.intval($mailid);
            ...
            $countQuery = 'SELECT a.listid, count(b.subid) as nbsub FROM `'.$this->getPrefix().'user_list` as a LEFT JOIN `'.$this->getPrefix().'user` as b ON a.user_id = b.user_id WHERE '.$confirmed.'  a.`list_id` IN ('.implode(',',array_keys($lists)).') GROUP BY a.`list_id`';
