...
class WYSIJA_model_queue extends WYSIJA_model{
    ...
    function queueCampaign($campaignid){
        ...
        $query="INSERT IGNORE INTO ".$this->getPrefix()."queue (`email_id` ,`user_id`) ";
        $query.="SELECT ".$data['email']['email_id'].", A.user_id  FROM ".$this->getPrefix()."user_list as A JOIN ".$this->getPrefix()."user as B on A.user_id=B.user_id WHERE B.status>".$statusmin." AND A.list_id IN (".implode(",",$data['campaign']['lists']['ids']).")  ";
//dbg($query);
        $this->query($query);
