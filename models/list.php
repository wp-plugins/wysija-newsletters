...
class WYSIJA_model_list extends WYSIJA_model{
    ...
    function getLists($id=false){
        ...
        if($id){
            ...
            $qry1="SELECT count(distinct A.user_id) as nbsub,A.list_id FROM `".$this->getPrefix()."user_list` as A LEFT JOIN `".$this->getPrefix()."user` as B on A.user_id=B.user_id WHERE list_id IN (".implode(',',$listids).") AND B.status >0 GROUP BY list_id";
