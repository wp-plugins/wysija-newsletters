...
class WYSIJA_help_wj_engine extends WYSIJA_object {
    ...
    function renderEmail($subject = NULL) {
        // fixes errors with preg_replace
    ...
    function renderEmailBody() {
        ...
        foreach($blocks as $key => $block) {
            ...
            $inline_block = preg_replace(array_keys($classes), $classes, $block);

            // check if the inline process worked, otherwise, leave block untouched
            if($inline_block !== NULL && strlen(trim($inline_block)) > 0) {
                $block = $inline_block;
            }
    ...
    function renderEmailFooter() {
        ...
        $data = array(
            'unsubscribe_link' => '<a href="[unsubscribe_link]" target="_blank">'.__("Unsubscribe",WYSIJA)."</a>",
            'company_address' => $config->getValue('company_address')
        );
