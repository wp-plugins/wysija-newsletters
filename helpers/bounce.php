  ...
  function handleMessages(){
    ...
    @flush();
      ...
      if(!empty($resultsVars[2])) $this->_message->email_id = $resultsVars[2];
		...
		//We didn't find it with the new powerful and cool way... so let's try with an other way
      /*if(empty($this->_message->user_id)){
		preg_match('#X-Subid *:? *([0-9]*)#i',$this->_message->analyseText,$resultsSubid);
		if(!empty($resultsSubid[1])) $this->_message->user_id = $resultsSubid[1];
      }

		if(empty($this->_message->email_id)){
			preg_match('#X-Mailid *:? *([0-9]*)#i',$this->_message->analyseText,$resultsMailid);
			if(!empty($resultsMailid[1])) $this->_message->email_id = $resultsMailid[1];
		}*/
  ...
  function _actionuser(&$oneRule){
        ...
        if(isset($oneRule->action_user['sub']) && !empty($oneRule->action_user['subscribeto'])){
                ...
                if(empty($status[$listId])){
                        $this->listsubClass->addSubscription($this->_message->user_id,array('1' => array($listId)));
                }elseif($status[$listId]->status != 1){
                        $this->listsubClass->updateSubscription($this->_message->user_id,array('1' => array($listId)));
                }
        }*/
