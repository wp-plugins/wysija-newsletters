...
class WYSIJA_help_user extends WYSIJA_object{
    ...
    function subscribe($user_id,$status=true,$auto=false){
        ...
        if($status){
            ...
            //$msg=__("User unsubscribed.",WYSIJA);
        }
        $modelUser=&WYSIJA::get("user","model");
    ...
    function sendConfirmationEmail($user_ids,$sendone=false){
        ...
        if(!$sendone)$this->notice(sprintf(__('%1$d emails have been sent to unconfirmed users.',WYSIJA),count($users)));
    ...
    function delete($user_ids,$sendone=false){
        ...
        else    $this->notice(sprintf(__(' %1$s user has been deleted.',WYSIJA),  implode(',', $emails)));
