...
class WYSIJA_help_user extends WYSIJA_object{
    ...
    function addSubscriber($data,$backend=false){
        ...
        if($user_get){
            ...
            if((int)$user_get['status']<1){
                ...
                $subscribe_to_list=0;
                if(!$dbloptin)  $subscribe_to_list=time();
            ...
            if(!empty($sendConfForIds)){
                ...
                if(($dbloptin && $subscriber_status) || !$dbloptin) $subscribe_to_list=time();
    ...
    function sendAutoNl($user_id,$params_based_on_type=false,$process_type='subs-2-nl',$added_by_admin=false){
        ...
        if(!$added_by_admin && !$modelUser->exists(array('user_id'=>$user_id,'status'=>$dbloptin))) return false;
        $userListss=array();
        ...
        foreach($emails as $key=> $email){
            if($email['params']['autonl']['event']!=$process_type) continue;
            switch($process_type){
                case 'subs-2-nl':
    ...
    function removeFromLists($listids = array(),$userids = array(), $is_batch_select = false){
        if(empty($userids) || (empty($listids) && empty($userids))) return true;
        if($is_batch_select)
    ...
    /**
     * refresh the total count of subscribers
     * @return boolean
     */
