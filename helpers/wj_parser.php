...
/**
 * @class Wysija Parser Helper (PHP4 version)
 */
    ...
    protected function _modifier ($string, $value, $vars=null)
    {
        ...
        foreach ($commands as $command) {
            ...
            switch ($command) {
                ...
                /*case 'replace':
                    if (preg_match ("'\"(.*?)\",\"(.*?)\"'", $argument, $preg)) {
                        $value = str_replace ($preg[1], $preg[2], $value);
                    }
                break;*/
                ...
                /*case 'format_date':
                    if ($value && $arguments) {
                        ...
                        $date = x;
                        $value = $date->toString ($arguments[0], null, $locale);
                    }
                break;*/
                ...
                case 'sprintf':
                    if ($arguments) {
                        $string_eval = "\$value = sprintf (\$value, '".implode ("','", $arguments)."');";
                        eval ($string_eval);
                    }
                break;
