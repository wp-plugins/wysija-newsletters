...
class WYSIJA_help_queue extends WYSIJA_object{
	...
	function _deleteQueue($queueDelete){
		...
		foreach($queueDelete as $email_id => $subscribers){
			...
			//$query = 'DELETE FROM `'.$modelQ->getPrefix().$modelQ->table_name.'` WHERE email_id = '.intval($email_id).' AND user_id IN ('.implode(',',$subscribers).') LIMIT '.$nbsub;
			...
			if(!$res){
				...
				if($nbdeleted != $nbsub){
					$status = false;
					$this->_display(__("Send process running in parallel detected, the system stopped but you will be able to resume it",WYSIJA));
				}
			}
		}
	...
	function _handleError(){
		...
		$message .= '<br/>';
		$message .= __("The system kept all non delivered e-mails in the queue, so you will be able to resume the send process later",WYSIJA);
		$message .= '<br/>';
		if($this->report){
			if(empty($this->successSend) AND empty($this->start)){
				$message .= __("Please verify your mail configuration and make sure you can send a test of this e-mail",WYSIJA);
				$message .= '<br/>';
				$message .= __("If you recently, successfully, sent a lot of e-mails, those errors may also be due to your server limitations",WYSIJA);
			}else{
				$message .= __("Your server apparently refuses to send more e-mails",WYSIJA);
				$message .= '<br/>';
				if(!acymailing_level(1)){
					$message .= __("Using one of our commercial versions, the system would be able to continue automatically the send process using a Cron",WYSIJA);
				}else{
					$message .= __("If you configured a cron task, the system will automatically continue the send process",WYSIJA);
				}
			}
		}
