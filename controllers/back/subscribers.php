...
class WYSIJA_control_back_subscribers extends WYSIJA_control_back{
    ...
    function save(){
        ...
        $this->redirect('admin.php?page=wysija_subscribers&id='.$id.'&action=edit');
        return true;
    }
    ...
    function defaultDisplay(){
        ...
        foreach($listsDB as $listobj){
            $lists[$listobj["list_id"]]['users']=$listobj['users'];
        }
       ...
       foreach($this->data['subscribers'] as $keysus=>$subscriber){
        ...
        if(!$this->data['subscribers']){
            $this->notice(__("Yikes! Couldn't find any subscriber.",WYSIJA));
        }
    ...
    function importpluginsave($id=false){
        ...
        $pluginsImportable=$modelConfig->getValue("pluginsImportable");
        $pluginsImported=array();
        foreach($_REQUEST['wysija']['import'] as $tablename =>$result){
            $plugInfo=$pluginsImportable[$tablename];
            if($result){
        ...
        $modelConfig->save(array("pluginsImported"=>$pluginsImported));
    ...
    function importplugins($id=false){
        ...
        $this->data=array();
        $this->data['plugins']=$modelConfig->getValue("pluginsImportable");
        $importedOnes=$modelConfig->getValue("pluginsImported");
        if($importedOnes){
            foreach($importedOnes as $tablename){
                unset($data['plugins'][$tablename]);
            }
        }
    ...
    function importmatch(){
        $this->js[]='wysija-validator';
        $bytes=WYSIJA::get_max_file_upload();
        ...
        if($_POST['wysija']['import']['type']=="copy"){
            if(!isset($_POST['wysija']['user_list']['csv'])){
                ...
                $this->error(__("The list you've pasted is to big for the browser. <strong>Upload the file</strong> instead.",WYSIJA),true);
                $this->redirect('admin.php?page=wysija_subscribers&action=import');
                return false;
            }
    ...
    function importsave(){
        ...
        foreach($csvChunks as $arra){
            ...
            $this->_importRows($queryStart,$arra,$j,$datatoinsert,$emailKey);
            $j++;
        }
    ...
    function exportcampaign(){
        ...
        $_REQUEST['wysija']['user']['user_id']=unserialize(base64_decode($_REQUEST['user_ids']));
        $this->exportlist();
    }
    ...
    function _getForm($id=false){
        if($id){
            ...
            $array=array('name'=>'','list_id'=>'','description'=>'');
            return $array;
        }
