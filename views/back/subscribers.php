...
class WYSIJA_view_back_subscribers extends WYSIJA_view_back{
    ...
    var $title="Lists and Subscribers";
    var $icon="icon-users";
    ...
    function menuTop($case=false){
        ...
        $arrayTrans=array("backtolist"=>__("Back to lists",WYSIJA),"back"=>__("Back",WYSIJA),"add"=>__('Add Subscriber',WYSIJA),"addlist"=>__('Add List',WYSIJA),"lists"=>__('Edit List',WYSIJA),"import"=>__('Import',WYSIJA),"export"=>__('Export',WYSIJA));
        switch($case){
            case "add":
            case "edit":
            case "export":
                            ...
                            foreach($data['list'] as $list){
                                ...
                                if(isset($valuefield[$list['list_id']])) {
                                    ...
                                    if($valuefield[$list['list_id']]['unsub_date']>0){
                                        ...
                                        $extratext=' <strong class="needInfo" alt="'.__('The user unsubscribed to this list. He needs to subscribe himself to be added to this list.You cannot forcefully add him yourself!',WYSIJA).'">'.__('Why can’t I add him?',WYSIJA).'</strong> ';
                                    }else{
                                        $checked=true;
                                    }
                                }
        ...
        if($modelC->getValue("premium_key")){
                ...
                <div id="wysistats3" class="left">
                    ...
                    <ol>
                        ...
                        foreach($data['clicks'] as $click){
                            echo "<li><em>".$click['name']."</em> : <strong >".sprintf(_n('%1$s hit', '%1$s hits', $click['number_clicked']), $click['number_clicked'])."</strong> - ".$click['url']."</li>";
                        }
            ...
            if(count($data['charts']['stats'])>0 ){
                ...
                echo str_replace(
                        ...
                        __("We have detailed statistics concerning this user, if you want to consult it [link]Go premium now![/link]",WYSIJA));
                echo '</p>';
            }
    ...
    function import($data){
        ...
        $data = WYSIJA::get_max_file_upload();
        $bytes=$data['maxmegas'];
                    ...
                    <tbody class="list:<?php echo $this->model->table_name.' '.$this->model->table_name.'-list" id="wysija-'.$this->model->table_name.'"' ?>>
                        ...
                        <?php
                            ...
                            foreach($data['csv'] as $columns){
                                ...
                                if($i==0){
                                     $valuefrow='';
                                    if(isset($this->data['firstrowisdata'])){
                                        $valuefrow='<input value="1" type="hidden" id="firstrowdata" name="firstrowisdata"  />';
                                    }else{
                                ...
                                foreach($columns as $val){
                                   if($i==0)  echo '<td><strong>'.$val.'</strong></td>';
                                   else echo '<td>'.$val.'</td>';
                                }
                                echo "</tr>";
