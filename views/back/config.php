...
class WYSIJA_view_back_config extends WYSIJA_view_back{
            ...
            <form name="wysija-settings" method="post" id="wysija-settings" action="" class="form-valid" autocomplete="off">
                ...
                <div id="bounce">
                    ...
                    if(!$config->getValue("premium_key")){
                        echo str_replace(array('[link]','[/link]'),array('<a class="wysija-premium" href="javascript:;" title="'.__("Purchase Wysija PREMIUM",WYSIJA).'">','<img alt="loader" src="'.WYSIJA_URL.'/img/wpspin_light.gif" /></a>'),__("[link]Purchase the premium version[/link] and get access to the <strong>Automated Bounce Handling</strong> system.",WYSIJA));
                    }else {
                        $this->bounce();
                    } ?>
                </div>
    ...
    function basics(){
        $step=array();
        if(defined('WYSIJA_DBG_ALL') && WYSIJA_DBG_ALL){
            $step['premium_key']=array(
            'type'=>'input',
            'label'=>__('Premium Account Serial Key',WYSIJA),
            'desc'=>__('Get immediate support, add your own marketing tracking codes, enable automated bounce handling. [link]Click here to purchase.[/link]',WYSIJA),
            'link'=>'<a class="wysija-premium" href="javascript:;" title="'.__("Purchase the premium version.",WYSIJA).'">');
        }
        ...
        if($modelC->getValue("premium_key")){
            ...
            $link='<a class="wysija-premium" href="javascript:;" title="'.__("Find out what's on offer.",WYSIJA).'">';
            $desc=str_replace(
                    array("[link]","[/link]"),
                    array($link,'<img src="'.WYSIJA_URL.'/img/wpspin_light.gif" alt="loader"/></a>'),$desc);
        }
        ...
        <table class="form-table">
            <tbody>
                <tr>
                    <th scope="row">
                        <p class="title"><?php echo $title ?></p><p class="description"><?php echo $desc ?> </p>
                    </th>
                    <td></td>
                </tr>
                ...
                <tr>
                    <th scope="row" class="row">
                        <?php 
                            ...
                            $field.='<p>'.__('Second best option for small lists. Your host sets the limit of emails per day.',WYSIJA).'</p>';
                            echo $field;
                        ?>
                    </th>
                    <th scope="row" class="row">
                        <?php 
                            ...
                            $field.=$formsHelp->radio(array("id"=>$id,'name'=>'wysija[config]['.$key.']'),$value,$checked);
                            $field.='Gmail</label>';
                            $field.='<p>'.__("The simplest of all solutions. Limited to 500 emails a day. We recommend that you open a dedicated Gmail account for this purpose.",WYSIJA).'</p>';
                            echo $field;
                        ?>
                    </th>
                    <th scope="row" class="row">
                        <?php 
                            ...
                            $field.='<p>'.__('You can use your own mail server or use a professional emailing SMTP. Good idea for lists over 2500 subscribers. We negotiated promotional offers with a few providers for you.',WYSIJA).'<a href="#">'.__('Find out more.',WYSIJA).'</a></p>';
                            echo $field;
                        ?>
                    </th>
                ...
                <tr class="hidechoice choice-sending-method-smtp choice-sending-method-site choice-sending-method-gmail">
                    <th scope="row">
                        <?php 
                            ...
                            //$field.='<p class="description">'.__("Leave this setting to 70 emails per 15 minutes, if you're unsure. To increase this value, get in touch with your host to find out the limit per hour of emails sent.",WYSIJA).'</p>';
                            echo $field;
                        ?>
                    </th>
                    <td colspan="2">
    ...
    function connection(){
     ...
     function rules(){
        ...
        foreach($arrayList as $list){
            $valuesDDP["unsub_".$list['list_id']]=sprintf(__('Unsubscribe the user and add him to the list "%1$s" '),$list['name']);
        }
        ...
        foreach($rules as $rule){
                ...
                if(isset($rule['action_user_min']) && $rule['action_user_min']>0){
                    $label.=' '.sprintf(__('(after %1$s try)',WYSIJA),$rule['action_user_min']);
                }
        ...
        foreach($step2 as $row =>$colparams){
            ...
            if(isset($colparams['forward'])){
                ...
                $formFields.=$formHelp->dropdown(array('id'=>$row, 'name'=>'wysija[config]['.$row.']'), $colparams['values'], $value, 'style="width:150px;"');
            }
    ...
    function advanced(){
        ...
        $step['role_campaign']=array(
            ...
            'desc'=>__('These are based on Wordpress user accounts.',WYSIJA));
